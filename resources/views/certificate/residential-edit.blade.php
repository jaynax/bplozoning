...
@section('title', 'Edit Residential Certificate')

@section('header-title', 'Edit Residential Certificate')
...
<div class="min-h-screen">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white rounded-lg shadow-lg p-8">
            <div class="mb-8 text-center">
                ...
                <h1 class="text-3xl font-bold text-gray-900">Edit Residential Zoning Certificate</h1>
                <p class="text-gray-600 mt-2">Update residential property details for zoning certification</p>
            </div>
            ...
            <form action="{{ route('certificate.update', $id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="property_type" name="property_type" required 
                                ...
                                <option value="Single Family House" {{ ($property_type ?? '') == 'Single Family House' ? 'selected' : '' }}>Single Family House</option>
                                ...
                                <option value="Condominium" {{ ($property_type ?? '') == 'Condominium' ? 'selected' : '' }}>Condominium</option>
                                <option value="Townhouse" {{ ($property_type ?? '') == 'Townhouse' ? 'selected' : '' }}>Townhouse</option>
                                <option value="Residential Lot" {{ ($property_type ?? '') == 'Residential Lot' ? 'selected' : '' }}>Residential Lot</option>
                                <option value="Other" {{ ($property_type ?? '') == 'Other' ? 'selected' : '' }}>Other</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="occupancy_type" name="occupancy_type" required 
                                ...
                                <option value="Owner-Occupied" {{ ($occupancy_type ?? '') == 'Owner-Occupied' ? 'selected' : '' }}>Owner-Occupied</option>
                                <option value="Rental" {{ ($occupancy_type ?? '') == 'Rental' ? 'selected' : '' }}>Rental</option>
                                <option value="Mixed-Use" {{ ($occupancy_type ?? '') == 'Mixed-Use' ? 'selected' : '' }}>Mixed-Use</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-between pt-6">
                    ...
                    <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-medium py-3 px-8 rounded-lg transition duration-200 flex items-center">
                        ...
                        Update Residential Certificate
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        async function loadBorders() {
            try {
                // Fetch available borders from server
